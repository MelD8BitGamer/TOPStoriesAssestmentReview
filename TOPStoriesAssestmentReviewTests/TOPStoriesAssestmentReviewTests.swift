//
//  TOPStoriesAssestmentReviewTests.swift
//  TOPStoriesAssestmentReviewTests
//
...
import XCTest
@testable import TOPStoriesAssestmentReview
//command + U to test Your UNIT TESTS if you get a check mark it works if not you will see it in the Test Navigator 5 from the document outline
class TOPStoriesAssestmentReviewTests: XCTestCase {
    //unit test for our topStories model
    func testTopStory() {
        //get the JSON from the postman
        //arrange
        let jsonData = """
{
    "status": "OK",
...
""".data(using: .utf8)!
        ...
        //if this is not the same as the expected title it will caus ethe test to fail. try erasing "Law" from the expectedString
        ...
        do {
            ...
            let subTitle = topStories.results.first?.title ?? ""
            XCTAssertEqual(expectedTitle, subTitle)
        } catch {
